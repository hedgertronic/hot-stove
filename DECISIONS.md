...
The game's design log: every rule the original spec left undefined or the
build revised, with the reasoning that decided it.

**How to read this file.** It is chronological — early sections resolve gaps
in `docs/archive/SPEC.md`, later `## Round N` sections record how the shipped
game evolved past it. The LATEST word wins: where two entries disagree, the
later one (and the shipped app) is the authority, and known-superseded
entries carry an explicit **Superseded** note. For the current scoring
numbers themselves, `pipeline/scoring.py` and `app/src/lib/scoring.ts` are
the source of truth.

## Rules gaps

The numbered gaps below were resolved before Milestone 1, against SPEC and
the original mock (`design/cardstock-v2.html`).
...
## Open items — deferred and still standing

Collected from the later round reviews (this section absorbed a stray
`app/DECISIONS.md` that had drifted out of the chronology). Numbering
continues the deferred lists of rounds 20–21.
