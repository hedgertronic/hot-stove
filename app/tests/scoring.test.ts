...
describe("World Baseball Classic medals ride the Ring-chasing row", () => {
  ...
  it("a gold medal is +3 and a silver is +1, both in ringPoints", () => {
    // Series parity, rung for rung: gold = ring (3), silver = pennant (1).
    expect(score({ ...args, wbcChampions: 1 }).ringPoints).toBe(3);
    expect(score({ ...args, wbcRunnersUp: 1 }).ringPoints).toBe(1);
    expect(score({ ...args, wbcChampions: 3, wbcRunnersUp: 2 }).ringPoints).toBe(11);
  });
  ...
  it("medals land in ringPoints, never in the award or win columns", () => {
    ...
    expect(medaled.total).toBeCloseTo(plain.total + 6, 5);
  });
   ...
   * a ring with Houston, and that season is worth 3 + 3 = 6. */
  it("a ring and a gold medal in the same season stack to +6", () => {
    expect(score({ ...args, rings: 1, wbcChampions: 1 }).ringPoints).toBe(6);
    // 2017 Carlos Beltran on the pennant-loser ledger would be 1 + 3 = 4.
    expect(score({ ...args, pennants: 1, wbcChampions: 1 }).ringPoints).toBe(4);
  });
