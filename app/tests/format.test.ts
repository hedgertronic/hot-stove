...
describe("seedCode / parseSeedCode", () => {
  it("round-trips the uint32 seed range", () => {
    ...
    expect(seedCode(0xffffffff)).toBe("1Z141Z3"); // the range's full 7 chars
  });

  it("pads every code to 7 chars — one length wherever seeds are listed", () => {
    // Display-only padding: parseInt reads the leading zeros as insignificant,
    // so codes shared before the padding still resolve to the same seed.
    expect(seedCode(42)).toBe("0000016");
    expect(seedCode(0)).toBe("0000000");
    for (const s of [0, 1, 42, 0xffffffff]) expect(seedCode(s)).toHaveLength(7);
    expect(parseSeedCode("16")).toBe(parseSeedCode("0000016"));
  });
